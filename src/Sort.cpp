...
void SwapMem(char *mem1, char *mem2, size_t size) {
    ...
    while(size / sizeof(uint64_t)) {
		...
		*(uint64_t*) mem2 = tmp;
		...
		mem1 += sizeof(uint64_t);
		mem2 += sizeof(uint64_t);
	}
	...
	if (size / sizeof(uint32_t)) {
		...
		*(uint32_t*) mem2 = tmp;
		...
		mem1 += sizeof(uint32_t);
		mem2 += sizeof(uint32_t);
	}
	...
	if (size / sizeof(uint16_t)) {
		...
		*(uint16_t*) mem2 = tmp;
		...
		mem1 += sizeof(uint16_t);
		mem2 += sizeof(uint16_t);
	}
...
int InversedLetterStrCmp(const struct String *str1, const struct String *str2) {
	...
	while (idx1 > -1 && idx2 > -1) {
		while (!(IsCyrillic(str1->value[idx1]) || isalpha(str1->value[idx1])) && idx1 > -1) { idx1--; }
		while (!(IsCyrillic(str2->value[idx2]) || isalpha(str2->value[idx2])) && idx2 > -1) { idx2--; }
